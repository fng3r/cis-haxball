...
class YoutubeService:
    ...
    def search_channel_livestreams(self, channel_id):
        ...
        playlist_response = (
            self.youtube.playlistItems().list(part='snippet', playlistId=uploads_playlist_id, maxResults=10).execute()
        )
        ...
        video_response = (
            self.youtube.videos()
            .list(part='snippet,liveStreamingDetails,contentDetails,statistics', id=','.join(video_ids))
            .execute()
        )
    ...
    def get_videos_by_ids(self, video_ids: List[str]) -> List[Dict]:
        ...
        """
        if not video_ids:
            logger.debug('No video IDs provided to get_videos_by_ids')
            return []
        ...
        try:
            ...
            for i in range(0, len(video_ids), 50):
                ...
                video_response = (
                    self.youtube.videos().list(part='snippet,contentDetails,statistics', id=','.join(batch)).execute()
                )
