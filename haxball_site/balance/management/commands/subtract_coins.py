...
class Command(BaseCommand):
    help = 'Списать монеты с балансов пользователей из CSV файла'
                    ...
                    try:
                        ...
                        if dry_run:
                            self.stdout.write(
                                ...
                                + f'Списать {amount} монет с баланса пользователя {username} - "{description}"'
                            )
                        else:
                            _ = BalanceService.subtract_coins(user, amount, description)

                            self.stdout.write(
                                f'✓ Строка {row_num}: Списано {amount} монет с баланса пользователя {username}. '
